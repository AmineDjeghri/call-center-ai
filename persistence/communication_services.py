...
class CommunicationServicesVoice(IVoice):
    ...
    def __init__(self, config: CommunicationServicesModel, sound: SoundModel):
        ...
        self._call_client = CallAutomationClient(
    ...
    async def acreate(
        ...
        res = self._call_client.create_call(
        ...
        call.voice_id = res.call_connection_id  # Save the call ID
    ...
    async def aanswer(
        ...
        try:
            res = self._call_client.answer_call(
                callback_url=callback_url,
                cognitive_services_endpoint=self._config.endpoint,
    ...
    async def atransfer(
        ...
        client = self._client_from_call(call)
        try:
    ...
    async def ahangup(
        ...
        client = self._client_from_call(call)
        try:
            client.hang_up(is_for_everyone=everyone)
        except ResourceNotFoundError:
    ...
    async def aplay_text(
        ...
        try:
            for chunk in self._chuncks_from_text(text):
                client.play_media(
                    operation_context=context,
                    play_source=self._audio_from_text(
                        lang=lang,
                        style=style,
                        text=chunk,
                        voice=voice,
    ...
    def _client_from_call(self, call: CallStateModel) -> CallConnectionClient:
        assert call.voice_id, "Call has no voice ID"
        return self._call_client.get_call_connection(call_connection_id=call.voice_id)
